...
def get_stats(lustre_stats):
   ...
   metric = subprocess.Popen(['lctl', 'get_param', lustre_stats], stdout=subprocess.PIPE).communicate()[0]
   ...
   if 'snapshot_time' in metric:
      ...
      for item in metric.splitlines()[2::]:
          if '_bytes' not in item:
             stats_type, stats_requests, unit_str, reqs_str = filter(None, item.split(" "))
             print "Stats %s=%s %d" % (stats_type,stats_requests,timestamp)
          else:
             read_write_ops, events, samples_str, unit_str, min_rd_wr, max_rd_wr, sum_rd_wr = filter(None, item.split(" "))
             print "Read_Write_Events %s=%s,%s_min=%s,%s_max=%s,%s_sum=%s %d" % (read_write_ops,events,read_write_ops,min_rd_wr,read_write_ops,max_rd_wr,read_write_ops,sum_rd_wr,timestamp)
   else:
